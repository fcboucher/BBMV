...
find.mle_FPK_multiple_clades_same_V_different_sig2=function(model,method='Nelder-Mead',init.optim=NULL){
  ...
  par_names=c() 
  for (i in 1:length(trees)){par_names=c(par_names,paste('sigsq_tree_',i,sep=''))}
  par_names=c(par_names,'a','b','c')
  par=list()
  for (i in 1:length(trees)){par[[i]]=2*exp(opt$par[i]) ; names(par)[i]=par_names[i]}
  if (model$ncoeff==3){par$a=opt$par[(length(trees)+1)] ; par$b=opt$par[(length(trees)+2)] ; par$c=opt$par[(length(trees)+3)]}
  else if (model$ncoeff==2){par$b=opt$par[(length(trees)+1)] ; par$c=opt$par[(length(trees)+2)]}
  else if (model$ncoeff==1){par$c=opt$par[(length(trees)+1)]}
  else{}
