...
Optim_bBM_x4x2x=function(tree,trait,Npts=100,bounds=NULL,method='L-BFGS-B',init.optim=NULL){
...
Optim_bBM_bounds_fixed_potential=function(tree,trait,V,bounds=NULL,init.optim=NULL){
  if (is.null(init.optim)){
...
Optim_bBM_0_flex_pts_start=function(tree,trait,Npts=50,method='Nelder-Mead',start.point=c(log(var(trait)/(2*max(branching.times(tree)))),min(trait)-0.5*(max(trait)-min(trait)),max(trait)+0.5*(max(trait)-min(trait)))){ # SEEMS TO WORK
  ...
  x0=bounds[1]+(bounds[2]-bounds[1])*(which(tree_formatted2$Pos[[tree_formatted2$tab[i,1]]]==max(tree_formatted2$Pos[[tree_formatted2$tab[i,1]]]))-1)/(Npts-1)
  ACE=lapply(tree_formatted2$Pos,function(x){cbind(seq(from=bounds[1],to=bounds[2],length.out=length(tree_formatted2$Pos[[tree_formatted2$tab[i,1]]])),x)})
  res=list(par=list(bounds=bounds,sigsq=2*exp(opt$par[1]),root_value=x0),lnL=-opt$value,k=4,aic=2*(4+opt$value),aicc=2*(4+opt$value)+(2*4*(4+1))/(length(trait)-4-1),method=method,convergence=opt$convergence,message=opt$message,root_density=tree_formatted2$Pos[[tree_formatted2$tab[i,1]]],ACE=ACE)
  return(res)	
}
...
Optim_bBM_linear=function(tree,trait,Npts=100,bounds=NULL,method='L-BFGS-B',init.optim=NULL){
  if (is.null(init.optim)){
    init.optim=c(log(var(trait)/(2*max(branching.times(tree)))),0)
  }
  if (is.null(bounds)){
    bounds=c(min(trait),max(trait))
  }
  ...
  if (method=='L-BFGS-B'){
    opt=optim(par=init.optim,fn=fun,method='L-BFGS-B',lower=c(-10,-10),upper=c(10,10),hessian=FALSE)
  }
  if (method=='Nelder-Mead'){
    opt=optim(par=init.optim,fn=fun,method='Nelder-Mead',hessian=FALSE)
  }
  # dCoeff is log(sigma^2/2)
  # now retrieve the ML value of x0, using the ML of dCoeff
  tree_formatted2= tree_formatted
  ...
  x0=bounds[1]+(bounds[2]-bounds[1])*(which(tree_formatted2$Pos[[tree_formatted2$tab[i,1]]]==max(tree_formatted2$Pos[[tree_formatted2$tab[i,1]]]))-1)/(Npts-1)
  ACE=lapply(tree_formatted2$Pos,function(x){cbind(seq(from=bounds[1],to=bounds[2],length.out=length(tree_formatted2$Pos[[tree_formatted2$tab[i,1]]])),x)})
  res=list(par=list(bounds=bounds,sigsq=2*exp(opt$par[1]),c=opt$par[2],root_value=x0),lnL=-opt$value,k=5,aic=2*(5+opt$value),aicc=2*(5+opt$value)+(2*5*(5+1))/(length(trait)-5-1),method=method,convergence=opt$convergence,message=opt$message,root_density=tree_formatted2$Pos[[tree_formatted2$tab[i,1]]],ACE=ACE)
  return(res)	
}
...
Optim_bBM_quadratic=function(tree,trait,Npts=100,bounds=NULL,method='L-BFGS-B',init.optim=NULL){
  if (is.null(init.optim)){
    init.optim=c(log(var(trait)/(2*max(branching.times(tree)))),0,0)
  }
  if (is.null(bounds)){
    bounds=c(min(trait),max(trait))
  }
  ...
  if (method=='L-BFGS-B'){
    opt=optim(par=init.optim,fn=fun,method='L-BFGS-B',lower=c(-10,-10,-10),upper=c(10,10,10),hessian=FALSE)
  }
  if (method=='Nelder-Mead'){
    opt=optim(par=init.optim,fn=fun,method='Nelder-Mead',hessian=FALSE)
  }
  ...
  x0=bounds[1]+(bounds[2]-bounds[1])*(which(tree_formatted2$Pos[[tree_formatted2$tab[i,1]]]==max(tree_formatted2$Pos[[tree_formatted2$tab[i,1]]]))-1)/(Npts-1)
  ACE=lapply(tree_formatted2$Pos,function(x){cbind(seq(from=bounds[1],to=bounds[2],length.out=length(tree_formatted2$Pos[[tree_formatted2$tab[i,1]]])),x)})
  res=list(par=list(bounds=bounds,sigsq=2*exp(opt$par[1]),b=opt$par[2],c=opt$par[3],root_value=x0),lnL=-opt$value,k=6,aic=2*(6+opt$value),aicc=2*(6+opt$value)+(2*6*(6+1))/(length(trait)-6-1),method=method,convergence=opt$convergence,message=opt$message,root_density=tree_formatted2$Pos[[tree_formatted2$tab[i,1]]],ACE=ACE)
  return(res)	
}
...
####################################
####################################
# A general function that fits all mossible models: bounds fixed or not, all shapes of the potential 
fit_BBMV=function(tree,trait,Npts=50,method='Nelder-Mead',verbose=T,V_shape,bounds='estimate',init.optim=NULL){
