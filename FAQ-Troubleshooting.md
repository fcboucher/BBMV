...
This line actually serves to define the likelihood function. The potential *V(x)* is given through the *dMat* argument: we directly calculate the transition matrix between points on the grid from this potential. *SEQ* is a vector that is created above in the *lnL_FPK* function and that gives the value of points on the trait grid In the present form of the code, BLABLA
